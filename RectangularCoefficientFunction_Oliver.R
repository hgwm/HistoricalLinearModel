...
optimize_q<-function(grpnorms,Nq=500,Xmat_train,Xmat_val,y_train,y_val,hMat,vMat,w_h_best,w_v_best){
  ...
  #points(new_q[whichbestq2],new_R2[whichbestq2],col
...
# testvec<-35:40
# 
# simu_x_train<-simu_x[trainvec,]
# simu_x_val<-simu_x[valvec,]
# simu_x_test<-simu_x[testvec,]
# simu_x_full<-simu_x[c(trainvec,valvec),]
# 
# Xmat_train<-get_x(as.numeric(t(simu_x_train)),M,max_lag)
# Xmat_val<-get_x(as.numeric(t(simu_x_val)),M,max_lag)
# Xmat_test<-get_x(as.numeric(t(simu_x_test)),M,max_lag)
# Xmat_full<-get_x(as.numeric(t(simu_x_full)),M,max_lag)
# Xmat<-get_x(as.numeric(t(simu_x)),M,max_lag)
# 
# XXProd_train<-crossprod(Xmat_train)
# XXProd_val<-crossprod(Xmat_val)
# XXProd_test<-crossprod(Xmat_test)
...
# y_val<-as.numeric(t(Y_val))
# y_test<-as.numeric(t(Y_test))
# y_full<-as.numeric(t(Y[c(trainvec,valvec),]))
# y<-as.numeric(t(Y))
# 
# y_train<-y_train[max_lag:length(y_train)]
# y_val<-y_val[max_lag:length(y_val)]
# y_test<-y_test[max_lag:length(y_test)]
# y_full<-y_full[max_lag:length(y_full)]
# y<-y[max_lag,length(y)]
# 
# 
# XyProd_train<-crossprod(Xmat_train,matrix(y_train,ncol = 1))
...
set.seed(111)
To_Zero<-rep(F,K)
result<-optimize_hp(hp_range=list(c(10,20),c(-12,15)),XTX=XXProd_full,hTh=hProd,vTv=vProd,XTy=XyProd_full,Xval=Xmat_test
...
result<-optimize_hp(hp_range=list(c(10,20),c(-12,15)),XTX=XXProd_sparse,hTh=hProd_sparse,vTv=vProd_sparse,XTy=XyProd_sparse,Xval=Xmat_test
                    ,yval=y_test,ytrain = y_full,n_hp=n_hp,To_Zero = To_Zero)
...
beta<-runif(p,min=0.2,max=2)
sig_flag<-sample(c(0,1),p,replace=T,prob = c(1- prob_sig,prob_sig))
beta[sig_flag==0]<-0

y<- X %*% matrix(beta,ncol=1)
y<- y + rnorm(length(y),sd=sd(y)/2)

savedR2<-rep(0,p)
for(i in 1:p){
  mod<-lm(y~X[,order(beta)[p:i]])
  savedR2[i]<-1-sum(mod$residuals^2)/sum((y-mean(y))^2)
  print(i)
}
plot(savedR2)
